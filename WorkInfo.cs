...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName,activityId,recommend,recommentTime)" + "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn= new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName = @workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," + "userName=@userName,recomment=@recommend,recommendTime=@recommendTime where workId=@workId";
